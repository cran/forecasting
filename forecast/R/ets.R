...
etsmodel <- function(y, errortype, trendtype, seasontype, damped,
  alpha=NULL, beta=NULL, gamma=NULL, phi=NULL, lower, upper, opt.crit, nmse, bounds)
{
...
#    browser()
    ...
    return(list(aic=aic,mse=mse,amse=amse,fit=fred,residuals=ts(e$e,f=m,s=tsp.y[1]),fitted=ts(y-e$e,f=m,s=tsp.y[1]),
...
initparam <- function(alpha,beta,gamma,phi,trendtype,seasontype,damped,lower,upper)
{
    ...
    if(is.null(alpha))
    {
        if(is.null(beta) & is.null(gamma))
            ...
            alpha <- max(beta+0.001,1.001-gamma)
    ...
    if(is.null(beta))
    {
        if(trendtype !="N")
        {
            beta <- lower[2] + .01*(upper[2]-lower[2])
            if(beta > alpha)
